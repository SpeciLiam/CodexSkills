...
5. Run the LinkedIn lane first: use logged-in Chrome to check LinkedIn jobs with Liam's normal filter chips as the default starting point. Start from `Jobs`, `Past 24 hours`, and `Entry-level`, and when available also use the saved relevance chips visible in Liam's usual view such as `Remote`, `Frontend`, `Gaming`, `Web`, `Java`, `Easy Apply`, `Employment type`, `Company`, `Under 10 applicants`, and `In my network`. Treat those extra chips as strong ranking and targeting hints, not absolute requirements when they would hide otherwise strong early-career SWE roles. Continue paging through fresh results as long as the roles remain within the last 24 hours and are still reasonably aligned with Liam's target role family. Do not stop at 1-2 pages if additional fresh, plausible roles remain.
