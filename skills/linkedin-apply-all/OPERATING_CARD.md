...
7. Prefer existing tailored resumes. If a realistic new posting has no valid tailored resume, follow `runPolicy.missingResumePolicy`: default is `tailor`, which means record `needs_tailoring`/`in_progress_tailoring`, run a bounded resume-tailor worker for that exact posting, refresh tracker/cache, then continue with a fresh application worker using the new resume. Use `queue_for_tailoring` only when Liam explicitly asks to collect postings without tailoring now.
...
12. Continue past duplicates, bad fits, tailored-then-applied items, queued-tailoring items from explicit queue-only runs, and per-application blockers. Stop only for systemic LinkedIn/Chrome/auth/rate-limit problems, exhausted results, user limits, or manual-blocker circuit breaker.
