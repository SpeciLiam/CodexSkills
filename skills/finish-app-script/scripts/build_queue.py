#!/usr/bin/env python3
"""Build the finish-app-script application queue and durable run state.

Forked from skills/finish-applications/scripts/build_application_queue.py with
three corrections:

1. "not automation-accessible" moved from TRUE_MANUAL_BLOCKERS to
   RETRYABLE_MANUAL_BLOCKERS — it's an environment failure from a prior run,
   not a permanent application-side blocker.
2. true_manual_reason() returns "" instead of a generic non-empty fallback
   when no `Manual apply needed:` note exists in the row's notes — so rows
   with stale Manual-Apply-Needed status but no specific reason re-enter the
   queue as retry/apply.
...
DEFAULT_RUN_STATE = Path("/tmp/fa_script_run_state.json")
OPERATING_CARD = ROOT / "skills" / "finish-app-script" / "OPERATING_CARD.md"
...
# True manual blockers: actual application-flow failures the agent cannot resolve.
...
# Retryable manual blockers: previous-run notes that don't indicate a real
# permanent block. The agent should attempt these again with fresh state.
...
def true_manual_reason(app: dict[str, Any]) -> str:
    ...
    if is_retryable_manual(app) or "bad-resume fix" in notes or "clean regenerated resume" in notes:
        return ""
    # FIX: when there is no `Manual apply needed:` note at all, treat the row
    # as retryable rather than permanently manual. The original returned a
    # non-empty generic fallback string here, which forced action=manual.
    if not reason:
        return ""
...
def build_run_state(
    ...
    for index, queue_entry in enumerate(agent_queue, start=1):
        ...
        item = {
            ...
            "notes": queue_entry.get("notes", ""),
            "result": previous.get("result", ""),
            "blocker": previous.get("blocker", ""),
...
def print_text(queues: dict[str, list[dict[str, Any]]]) -> None:
    print("Application Queue (finish-app-script)")
    print("=====================================")
    print("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the finish-app-script application queue.")
    ...
    parser.add_argument(
        ...
        help=f"Path for durable run state; default: {DEFAULT_RUN_STATE}",
    )
    ...
    parser.add_argument("--date", default=date.today().isoformat(), help="Date for manual Workday notes (YYYY-MM-DD)")
