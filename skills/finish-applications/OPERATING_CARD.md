...
2. Standing answers are pre-approved. Use name, email, phone, location, authorization, sponsorship, demographics, NYC/SF/hybrid/in-office openness, start-date availability, salary guidance, saved profile fields, and prior answered same-question patterns from tracker notes/submitted rows without asking when confidence is high.
3. Tailored resume means pre-approval to attempt. A row with `resumePdf` set means Liam has approved the attempt. Fill safe fields, upload the resume, and proceed.
   Regenerated rows marked with `bad-resume fix` / `Clean regenerated resume ready` are retry/apply candidates, even if an older note says manual. Every queued row gets an attempt as far as safely possible; account creation/login, CAPTCHA, 2FA, Workday account/profile gates, non-routine legal signature/contract terms, and other true hard blockers become precise manual handoffs only after the attempt.
