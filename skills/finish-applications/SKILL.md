...
- Do not re-ask or re-block on questions Liam has already answered in previous
  applications. Use `skills/linkedin-easy-apply-nodriver/references/application-defaults.md`,
  `application-trackers/applications.md`, and current row notes as the answer
  bank for repeated routine questions.
...
- For FRQ/custom written answers, it is okay not to submit. Draft the answer
  first, fill every other safe field, leave the tab open at the cleanest
  pre-submit point for Liam review, record the exact FRQ/review item, and
  continue.
- In the handoff/chat report, say why the application was not submitted and
  include the exact FRQ question plus draft. If Liam approves that FRQ answer in
  chat, a follow-up pass may submit the prepared open tab, capture confirmation,
  update tracker/cache, close the tab, and mark the row applied.
...
Do not spawn subagents, explorers, workers, or parallel browser agents for application flows. Use Chrome through Computer Use directly in the current agent unless Liam explicitly asks for a different browser or Chrome is unavailable. Direct ATS rows still go through Chrome Computer Use by default, because Chrome is the normal authenticated and file-upload environment for this workflow. Use Liam's Chrome profile for actual applications: profile name `Liam`, account `liamvanpj@gmail.com`, profile directory `Default`. Ben's profile (`bendov1010@gmail.com`, `Profile 1`) is only for LinkedIn sourcing/list building, not for filling or submitting applications. If Chrome is on Ben, open Liam's profile with `open -na "Google Chrome" --args --profile-directory="Default"` or switch profiles before proceeding.
...
5. Submit only when ready.
   ...
   - Confidence is high when all required answers are covered by Liam's tracker, resume, profile, standing answers, prior answered same-question patterns, or routine acknowledgement defaults and no blocker from the previous section is present.
...
- Workday rows should still be attempted as far as safely possible. Leave them for Liam only when Workday presents account/profile/legal/unknown-answer blockers after the attempt.
