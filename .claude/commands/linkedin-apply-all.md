...
description: "Drain Liam Van's LinkedIn software engineer search results one by one from Chrome, using a provided LinkedIn jobs search URL or the salary-filtered last-24-hours SWE search, skipping duplicate/already-applied postings, tailoring or reusing resumes, submitting safe applications, recording precise blockers, updating the markdown tracker/cache, and coordinating with Codex through tandem review without letting two agents operate the browser at once."
---

# LinkedIn Apply All

Use this command as Claude's entrypoint for the repository skill at
`skills/linkedin-apply-all/SKILL.md`. When working with Codex, prefer the tandem
pattern in `skills/linkedin-apply-all/references/tandem-usage.md`: Codex owns
Chrome, applications, tracker/cache writes, and final reconciliation; Claude
critiques the plan and reviews diffs.

Use this skill when Liam wants Codex to walk a LinkedIn jobs search result list
in order and keep applying until the list is exhausted, saturated with
duplicates, or blocked by platform/application gates. This skill is an
orchestrator: reuse the existing recruiting skills and scripts instead of
forking their logic.
...
When Liam asks to use Codex too, use the `tandem` skill with Codex as the
browser/apply driver and Claude as read-only plan/review peer. Read
`references/tandem-usage.md`.

Never let Claude and Codex both operate Chrome or edit the same tracker files at
the same time. Claude should critique the run plan, review duplicate criteria,
review tracker diffs, and call out safety gaps. Codex should own Chrome,
...
- Whether tandem Claude critique/review succeeded or was unavailable.
